use stellr_core::{
    ...
    MilestoneRef, Model, Provider, ProviderError, ProviderSnapshot, RepoRef, SpaceModel,
};
...
impl Provider for CurrentOnlyProvider {
    async fn fetch(&self, _repo: &RepoRef) -> Result<ProviderSnapshot, ProviderError> {
        Ok(ProviderSnapshot::without_viewer(Vec::new()))
    }
}
...
async fn current_only_providers_get_a_backward_compatible_snapshot() {
    ...
    assert_eq!(snapshot, ProviderSnapshot::without_viewer(Vec::new()));
}
